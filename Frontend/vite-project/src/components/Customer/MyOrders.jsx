    ...
    <div style={{ padding: "16px 0 4px" }}>
      ...
      {order.status === "Delivered" ? (
        ...
        <div style={{ ...S.banner, background: "#eff6ff", color: "#1d4ed8" }}>
          ...
          Expected delivery inside 3–5 business days
        </div>
      )}
    </div>
  );
...
const OrderCard = ({ order, onTrack, onCancel, expanded, onToggle }) => {
  ...
  const totalAmt = `₹${(order.total_amount || 0).toLocaleString()}`;
  const formattedDate = order.created_at ? new Date(order.created_at).toLocaleDateString() : "N/A";
    ...
    <div style={S.card}>
      ...
      <div style={S.cardHeader}>
        <div style={S.cardHeaderLeft}>
          ...
          <div style={S.orderMeta}>
            ...
            {formattedDate} &nbsp;•&nbsp; {itemsList.length || 1} item(s) &nbsp;•&nbsp; 
      ...
      <div style={S.itemsRow}>
        {itemsList.length > 0 ? (
          ...
          <div style={S.itemChip}>
            <div style={S.itemThumbPlaceholder}>
              <Icon name="package" size={18} color="#2563eb" />
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={S.itemName}>Package verification content logged</div>
              <div style={S.itemSubtext}>Recipient: {order.full_name || "Customer"}</div>
            </div>
          </div>
        )}
 
        {itemsList.length > 3 && (
          <div style={S.moreItems}>+{itemsList.length - 3} more items</div>
        )}
      </div>
          ...
          <div style={S.detailGrid}>
            <div>
              ...
              <div style={S.detailValue}>
                ...
                {order.address || "No address on record."}<br/>
            ...
            <div>
              ...
              <div style={S.detailValue}>{order.payment_method || "Cash On Delivery (COD)"}</div>
            </div>
          </div>
...
const OrderTrackingPage = ({ order, onBack }) => {
  const sc = getStatusColor(order.status || "Processing");
  const totalAmt = `₹${(order.total_amount || 0).toLocaleString()}`;
  const formattedDate = order.created_at ? new Date(order.created_at).toLocaleDateString() : "N/A";
  const itemsList = order.items || [];
        ...
        <div style={S.summaryCard}>
          ...
          <div style={S.summarySection}>
            ...
            {itemsList.length > 0 ? (
              ...
              <p style={S.summaryText}>Package details verified successfully.</p>
            )}
          </div>
          <div style={S.divider} />
...
const MyOrders = () => {
  ...
  const filteredOrders = filterStatus === "All" ? orders : orders.filter(o => (o.status || "Processing") === filterStatus);
...
const S = {
  ...
  stepperWrap: { padding: "0 20px 12px", borderBottom: "1px solid #f1f5f9" },
