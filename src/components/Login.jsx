...
import { useAuth } from '../context/AuthContext'; // ✅ Activated
import { useNavigate } from 'react-router-dom';
 
const Login = () => {
  const navigate = useNavigate();
  const { login } = useAuth(); // ✅ Destructured login function from context
    ...
    // --- SUBMIT LOGIN ---
      ...
      if (email === 'admin@shop.com' && password === 'admin123') {
        ...
        setTimeout(() => {
          ...
          navigate('/dashboard'); // ✅ Points to your AppRoutes layout map
        }, 1500);
      } else {
...
const styles = {
  container: { display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '90vh', backgroundColor: '#f8fafc', fontFamily: 'system-ui, sans-serif', position: 'relative' },
  toast: { position: 'absolute', top: '20px', color: '#ffffff', padding: '12px 24px', borderRadius: '8px', fontSize: '15px', fontWeight: '500', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1)', zIndex: 1000 },
  card: { background: '#ffffff', padding: '35px', borderRadius: '12px', boxShadow: '0 4px 12px rgba(0,0,0,0.05)', width: '100%', maxWidth: '440px' },
  title: { margin: '0 0 6px 0', color: '#1e293b', fontSize: '24px', fontWeight: '600', textAlign: 'center' },
  subtitle: { margin: '0 0 24px 0', color: '#64748b', fontSize: '14px', textAlign: 'center' },
  errorBox: { backgroundColor: '#fef2f2', color: '#b91c1c', padding: '12px', borderRadius: '6px', fontSize: '14px', marginBottom: '16px', border: '1px solid #fee2e2' },
  ...
  submitBtn: { backgroundColor: '#4f46e5', color: '#ffffff', padding: '12px', borderRadius: '6px', border: 'none', fontSize: '15px', fontWeight: '500', cursor: 'pointer', marginTop: '10px' },
  toggleContainer: { marginTop: '20px', textAlign: 'center', fontSize: '14px' },
  toggleText: { color: '#64748b' },
  toggleLink: { background: 'none', border: 'none', color: '#4f46e5', fontWeight: '500', cursor: 'pointer', padding: '0', textDecoration: 'underline' }
};
